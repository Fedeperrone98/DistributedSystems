<link rel="stylesheet" href="css/nav_bar.css?v=1.3">

<% String my_username = (String) request.getSession().getAttribute("my_username"); %>

<ul id="navbar" class="navbar">
 <li id="profile" class="navitem"><a href="${pageContext.request.contextPath}/profile?username=${my_username}">Profile</a></li>
 <li id="meetings" class="navitem"><a href="#">Meetings</a></li>
 <li id="notification"><a href="#">&#128276;</a></li>
 <li id="logout" class="navitem"><a href="${pageContext.request.contextPath}/logout">Log Out</a></li>
</ul>
